...
Simulation::Simulation(Graph graph, vector<Agent> agents) : mGraph(graph), mAgents(agents), coalitions(), coalitionsMandates() {
	...
	for (Agent& agent : mAgents) {
        ...
        //agent.setCoalitionId(agent.getId());
...
/*      tiota
        from CTR
        vector<int> originalCoalition;
        originalCoalition.push_back(agent.getPartyId());
        coalitions.push_back(originalCoalition);
*/